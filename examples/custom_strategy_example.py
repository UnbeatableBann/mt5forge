...
class RSIMeanReversionStrategy(StrategyBase):
    ...
    def on_candle(self, symbol: str, tf: Timeframe, candles: pd.DataFrame) -> Signal | None:
        ...
        if len(candles) >= 200:
            ...
            rsi_val = rsi.iloc[-1] if isinstance(rsi, pd.Series) else rsi[-1]
            sma_val = sma.iloc[-1] if isinstance(sma, pd.Series) else sma[-1]
            if float(rsi_val) < 30 and price > float(sma_val):
                signal = Signal(SignalDirection.BUY, symbol, suggested_sl_pips=25, suggested_tp_pips=50)
            elif float(rsi_val) > 70 and price < float(sma_val):
